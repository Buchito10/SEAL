import { getToken, logout, type SessionUser } from "./auth";
...
export function createUser(input: CreateUserInput) {
  return apiFetch<{ user: AdminUser; resetLinkSent: boolean }>("/admin/users", {
    method: "POST",
...
export function verifyResetToken(token: string) {
  return apiFetch<{ valid: boolean }>(`/auth/password/verify?token=${encodeURIComponent(token)}`, {
    auth: false,
  });
}

export function confirmPasswordReset(input: ConfirmPasswordResetInput) {
  return apiFetch<{ changed: boolean; profile_completed: boolean }>("/auth/password/confirm", {
    method: "POST",
    auth: false,
    body: JSON.stringify(input),
  });
